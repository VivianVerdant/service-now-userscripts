/* globals getInputSelection getCaret autocorrect_dictionary setInputSelection */

function textarea_autocorrect_eventlistener(event) {
	if (this.getAttribute("placeholder") == "Work notes (Internal Only)") {
		return
	}
	
	event.stopPropagation();

    var content = this.value;

	if (content.length == 1) {
		this.value = content.toUpperCase();
		return
	}

	var sel = getInputSelection(this);
    var caret = getCaret(this);
    //console.warn("caret: ", caret);
    var pre_caret = content.substring(0,caret);
    var words = pre_caret.split(/(\s)/gm);
    words = words.filter((str) => {return !(str == "" || str == " " || str=="\n")});
    var word = words[words.length-1];
    //console.warn("word: ", word);
    if (autocorrect_dictionary[word]) {
        //console.warn("content: ", content);
        pre_caret = pre_caret.substring(0, pre_caret.length - word.length);
        //console.warn("pre_caret: ", pre_caret);
        var replacement = autocorrect_dictionary[word];
        pre_caret += replacement;
        //console.warn("substitution: ", pre_caret);
        this.value = pre_caret + content.substring(caret,content.length);
        content = this.value;
        setInputSelection(this, sel.start + replacement.length - word.length, sel.end + replacement.length - word.length);
    }
};

function assign_autocorrect_element(el, dict) {

    el.addEventListener("keyup", textarea_autocorrect_eventlistener);
}
